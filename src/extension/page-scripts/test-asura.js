export function testAsura () {
    const title = document.querySelector('h1, a[href="/"]+svg+h3, a[href="/series"]+a>h3')?.innerText?.trim()
    const url = window.location.pathname.match(/^\/(?:series|comics)\/[^/]*/)?.[0].replace(/-([a-z0-9]{8})$/, '-')
